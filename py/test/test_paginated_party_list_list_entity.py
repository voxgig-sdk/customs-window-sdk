...
class TestPaginatedPartyListListEntity:

    def test_should_create_instance(self):
        testsdk = CustomsWindowSDK.test(None, None)
        ent = testsdk.PaginatedPartyListList(None)
        assert ent is not None

    def test_should_run_basic_flow(self):
        setup = _paginated_party_list_list_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in []:
            _skip, _reason = runner.is_control_skipped("entityOp", "paginated_party_list_list." + _op, "live" if _live else "unit")
            if _skip:
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set CUSTOMSWINDOW_TEST_PAGINATED_PARTY_LIST_LIST_ENTID JSON to run live")
        client = setup["client"]
        ...
        paginated_party_list_list_ref01_data = None
        if len(paginated_party_list_list_ref01_data_raw) > 0:
            paginated_party_list_list_ref01_data = helpers.to_map(paginated_party_list_list_ref01_data_raw[0][1])



def _paginated_party_list_list_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/paginated_party_list_list/PaginatedPartyListListTestData.json")
    ...
    _entid_env_raw = os.environ.get(
        "CUSTOMSWINDOW_TEST_PAGINATED_PARTY_LIST_LIST_ENTID")
    ...
    idmap_resolved = helpers.to_map(
        env.get("CUSTOMSWINDOW_TEST_PAGINATED_PARTY_LIST_LIST_ENTID"))
