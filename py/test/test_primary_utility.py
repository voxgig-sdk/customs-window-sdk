...
from customswindow_sdk.core.result import CustomsWindowResult
from customswindow_sdk.core.response import CustomsWindowResponse
from customswindow_sdk.core.operation import CustomsWindowOperation
from customswindow_sdk.core.error import CustomsWindowError
...
def _err_from_map(m):
    ...
    if msg == "":
        return None
    code = m.get("code", "")
    return CustomsWindowError(code, msg)
...
class TestPrimaryUtility:
    ...
    def test_exists(self):
        client = CustomsWindowSDK.test(None, None)
    ...
    def test_done_basic(self):
        ...
        def subject(entry):
            ...
            except CustomsWindowError as e:
                return None, e
    ...
    def test_make_error_no_throw(self):
        ...
        ctx.result = CustomsWindowResult({
            "ok": False,
            "resdata": {"id": "safe01"},
        })
    ...
    def test_feature_init_basic(self):
        init_client = CustomsWindowSDK.test(None, None)
        ...
        class TestInitFeature(CustomsWindowBaseFeature):
            def __init__(self):
                super().__init__()
                self.name = "initfeat"
                self.active = True
    ...
    def test_feature_init_inactive(self):
        ...
        class TestInitFeatureInactive(CustomsWindowBaseFeature):
            def __init__(self):
                super().__init__()
                self.name = "nofeat"
                self.active = False
    ...
    def test_fetcher_blocked_test_mode(self):
        ...
        blocked_client = CustomsWindowSDK({
            "base": "http://localhost:8080",
            "system": {
                "fetch": mock_fetch,
            },
    ...
    def test_make_result_no_result(self):
        ...
        utility = client._utility
        ctx = _make_test_full_ctx(client, utility)
        ctx.spec = CustomsWindowSpec({"step": "start"})
        ctx.result = None
    ...
    def test_make_spec_basic(self):
        ...
        spec_client = CustomsWindowSDK.test(None, setup_opts)
    ...
    def test_operator_basic(self):
        ...
        def subject(entry):
            ...
            if not isinstance(in_val, dict):
                in_val = {}
            op = CustomsWindowOperation(in_val)
            return {
                "entity": op.entity,
                "name": op.name,
                "input": op.input,
    ...
    def test_prepare_auth_basic(self):
        ...
        auth_client = CustomsWindowSDK.test(None, setup_opts)
