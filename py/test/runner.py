...
class CustomsWindowTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            CustomsWindowTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = CustomsWindowTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = CustomsWindowTestRunner.getenv("CUSTOMS_WINDOW_TEST_LIVE")
        override = CustomsWindowTestRunner.getenv("CUSTOMS_WINDOW_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = CustomsWindowTestRunner.getenv(key)
        ...
        explain = CustomsWindowTestRunner.getenv("CUSTOMS_WINDOW_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["CUSTOMS_WINDOW_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if CustomsWindowTestRunner._test_control is not None:
            return CustomsWindowTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                CustomsWindowTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            CustomsWindowTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = CustomsWindowTestRunner.load_test_control()
...
def load_env_local():
    CustomsWindowTestRunner.load_env_local()


def env_override(m):
    return CustomsWindowTestRunner.env_override(m)


def entity_data(v):
    return CustomsWindowTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return CustomsWindowTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return CustomsWindowTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return CustomsWindowTestRunner.load_test_control()


def live_client_options():
    return CustomsWindowTestRunner.live_client_options()


def live_delay_ms():
    return CustomsWindowTestRunner.live_delay_ms()
