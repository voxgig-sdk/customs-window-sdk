...
describe("PartyDirect", function()
  it("should direct-list-party", function()
    local setup = party_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-party", setup.live and "live" or "unit")
  ...
  it("should direct-load-party", function()
    local setup = party_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-party", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function party_direct_setup(mockres)
  runner.load_env_local()

  local calls = {}

  local env = runner.env_override({
    ["CUSTOMSWINDOW_TEST_PARTY_ENTID"] = {},
