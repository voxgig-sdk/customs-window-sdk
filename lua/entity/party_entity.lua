...
local PartyEntity = {}
PartyEntity.__index = PartyEntity


function PartyEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, PartyEntity)
  self._name = "party"
  self._client = client
...
function PartyEntity:get_name()
  return self._name
end


function PartyEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return PartyEntity.new(self._client, opts)
end
...
function PartyEntity:mark_deleted()
  self._deleted = true
end


function PartyEntity:deleted()
  return true == self._deleted
end


function PartyEntity:data_set(args)
  if args ~= nil then
...
function PartyEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function PartyEntity:match_set(args)
  if args ~= nil then
...
function PartyEntity:match_get()
...
function PartyEntity:stream(action, args, callopts)
...
---@param reqmatch PartyLoadMatch
---@param ctrl? table
---@return Party
---@return string? err
function PartyEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqmatch PartyListMatch
---@param ctrl? table
---@return Party[]
---@return string? err
function PartyEntity:list(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "list",
    ctrl = ctrl,
...
---@param reqdata PartyCreateData
---@param ctrl? table
---@return Party
---@return string? err
function PartyEntity:create(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "create",
    ctrl = ctrl,
...
---@param reqdata PartyUpdateData
---@param ctrl? table
---@return Party
---@return string? err
function PartyEntity:update(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "update",
    ctrl = ctrl,
...
---@param reqmatch PartyRemoveMatch
---@param ctrl? table
---@return Party
---@return string? err
function PartyEntity:remove(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "remove",
    ctrl = ctrl,
...
function PartyEntity:_run_op(ctx, post_done)
  local utility = self._utility
