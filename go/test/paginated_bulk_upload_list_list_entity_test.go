...
func TestPaginatedBulkUploadListListEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		paginatedBulkUploadListListRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.paginated_bulk_upload_list_list")))
...
func paginated_bulk_upload_list_listBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("CUSTOMS_WINDOW_TEST_PAGINATED_BULK_UPLOAD_LIST_LIST_ENTID")
	...
	idmapResolved := core.ToMapAny(env["CUSTOMS_WINDOW_TEST_PAGINATED_BULK_UPLOAD_LIST_LIST_ENTID"])
