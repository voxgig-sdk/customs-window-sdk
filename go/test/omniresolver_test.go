...
import (
	...
	omni "github.com/voxgig-sdk/customs-window-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.CustomsWindowError); is {
				out := map[string]any{
					"name":    "CustomsWindowError",
...
func omniCtx(arg any, client *sdk.CustomsWindowSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.CustomsWindowSDK); is {
			client = live
