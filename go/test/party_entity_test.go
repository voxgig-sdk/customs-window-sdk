...
func TestPartyEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		partyRef01Data := core.ToMapAny(vs.GetProp(
			vs.GetPath(setup.data, []any{"new", "party"}), "party_ref01"))

		partyRef01DataResult, err := partyRef01Ent.Create(partyRef01Data, nil)
		if err != nil {
			t.Fatalf("create failed: %v", err)
		}
		partyRef01Data = core.ToMapAny(entityData(partyRef01DataResult))
		...
		partyRef01MarkdefUp0Name := "bank_details"
		...
		partyRef01ResdataUp0 := core.ToMapAny(entityData(partyRef01ResdataUp0Result))
		...
		partyRef01DataDt0LoadResult := core.ToMapAny(entityData(partyRef01DataDt0Loaded))
...
func partyBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("CUSTOMS_WINDOW_TEST_PARTY_ENTID")
	...
	idmapResolved := core.ToMapAny(env["CUSTOMS_WINDOW_TEST_PARTY_ENTID"])
