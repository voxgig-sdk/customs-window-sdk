...
func TestSubmissionDetailEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set CUSTOMSWINDOW_TEST_SUBMISSION_DETAIL_ENTID JSON to run live")
			return
		}
		client := setup.client
		...
		submissionDetailRef01Data = core.ToMapAny(submissionDetailRef01DataResult)
...
func submission_detailBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("CUSTOMSWINDOW_TEST_SUBMISSION_DETAIL_ENTID")
	...
	idmapResolved := core.ToMapAny(env["CUSTOMSWINDOW_TEST_SUBMISSION_DETAIL_ENTID"])
