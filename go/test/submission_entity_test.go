...
func TestSubmissionEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set CUSTOMSWINDOW_TEST_SUBMISSION_ENTID JSON to run live")
			return
		}
		client := setup.client
		...
		submissionRef01Data = core.ToMapAny(submissionRef01DataResult)
		...
		submissionRef01ResdataUp0 := core.ToMapAny(submissionRef01ResdataUp0Result)
		...
		submissionRef01DataDt0LoadResult := core.ToMapAny(submissionRef01DataDt0Loaded)
...
func submissionBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("CUSTOMSWINDOW_TEST_SUBMISSION_ENTID")
	...
	idmapResolved := core.ToMapAny(env["CUSTOMSWINDOW_TEST_SUBMISSION_ENTID"])
