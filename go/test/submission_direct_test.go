...
func TestSubmissionDirect(t *testing.T) {
	...
	t.Run("direct-load-submission", func(t *testing.T) {
		setup := submissionDirectSetup(map[string]any{"id": "direct01"})
		_mode := "unit"
		if setup.live {
			_mode = "live"
		}
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-load-submission", _mode); _shouldSkip {
			if _reason == "" {
				_reason = "skipped via sdk-test-control.json"
			}
			t.Skip(_reason)
...
func submissionDirectSetup(mockres any) *submissionDirectSetupResult {
	...
	env := envOverride(map[string]any{
		"CUSTOMS_WINDOW_TEST_SUBMISSION_ENTID": map[string]any{},
	...
	if live {
		...
		idmap := map[string]any{}
		if entidRaw, ok := env["CUSTOMS_WINDOW_TEST_SUBMISSION_ENTID"]; ok {
