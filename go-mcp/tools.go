...
type Args struct {
	Entity string         `json:"entity" jsonschema:"bulk_upload | file | paginated_bulk_upload_list_list | paginated_party_list_list | paginated_submission_list_list | party | submission | submission_detail"`
	Query  map[string]any `json:"query,omitempty" jsonschema:"optional match map e.g. {\"id\":1} for load, omit for list"`
}

func registerTools(server *mcp.Server, client *sdk.CustomsWindowSDK) {
	mcp.AddTool(server, &mcp.Tool{
		Name: "customs-window_list",
		Description: "List records from CustomsWindow. " +
	...
	mcp.AddTool(server, &mcp.Tool{
		Name: "customs-window_load",
		Description: "Load a single record from CustomsWindow. " +
...
func runOp(client *sdk.CustomsWindowSDK, op string, args Args) (*mcp.CallToolResult, any, error) {
...
func entityFor(client *sdk.CustomsWindowSDK, name string) (sdk.CustomsWindowEntity, error) {
	switch strings.ToLower(name) {
	case "bulk_upload":
		return client.BulkUpload(nil), nil
	case "file":
		return client.File(nil), nil
	case "paginated_bulk_upload_list_list":
		return client.PaginatedBulkUploadListList(nil), nil
	case "paginated_party_list_list":
		return client.PaginatedPartyListList(nil), nil
	case "paginated_submission_list_list":
		return client.PaginatedSubmissionListList(nil), nil
	case "party":
		return client.Party(nil), nil
	case "submission":
		return client.Submission(nil), nil
	case "submission_detail":
		return client.SubmissionDetail(nil), nil
