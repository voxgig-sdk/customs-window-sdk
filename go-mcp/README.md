...
[MCP](https://modelcontextprotocol.io) server exposing the CustomsWindow SDK as
two agent tools — `customs-window_list` and `customs-window_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/customs-window-mcp)
make build
...
claude mcp add --scope user customs-window \
  -- /absolute/path/to/customs-window-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./customs-window-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// customs-window_list: first page of records
{ "entity": "bulk_upload" }
{ "entity": "bulk_upload", "query": { } }

// customs-window_load: one record by id
{ "entity": "bulk_upload", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/customs-window-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user customs-window \
     -- "$PWD"/dist/*/customs-window-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `customs-window_list` and `customs-window_load` tools now appear
   in new sessions. Ask the agent to *"list bulk_upload using customs-window"*
   and it calls `customs-window_list` with `{"entity":"bulk_upload"}`.
...
### Call the `customs-window_list` tool
...
### Call the `customs-window_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `customs-window_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `customs-window_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
```sh
./customs-window-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"customs-window_load","arguments":{"entity":"bulk_upload","query":{"id":1}}}}'
```
