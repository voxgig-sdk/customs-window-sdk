...
function envOverride(m: Record<string, any>) {
  if (
    'TRUE' === process.env.CUSTOMSWINDOW_TEST_LIVE ||
    'TRUE' === process.env.CUSTOMSWINDOW_TEST_OVERRIDE
  ) {
  ...
  m.CUSTOMSWINDOW_TEST_EXPLAIN = process.env.CUSTOMSWINDOW_TEST_EXPLAIN || m.CUSTOMSWINDOW_TEST_EXPLAIN
