...
(0, node_test_1.describe)('BulkUploadDirect', async () => {
    ...
    (0, node_test_1.test)('direct-exists', async () => {
        const sdk = new __1.CustomsWindowSDK({
    ...
    (0, node_test_1.test)('direct-load-bulk_upload', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup({ id: 'direct01' });
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-load-bulk_upload', setup.live))
            return;
    ...
    (0, node_test_1.test)('direct-list-bulk_upload', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }]);
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-list-bulk_upload', setup.live))
            return;
        const { client, calls } = setup;
        const params = {};
        const query = {};
...
function liveScenariosActive() { return false && process.env.CUSTOMS_WINDOW_TEST_LIVE === 'TRUE'; }
function directSetup(mockres) {
    ...
    if (live) {
        ...
        const client = new __1.CustomsWindowSDK(Object.assign({}, (0, utility_1.liveClientOptions)(), { system: { fetch: transport.fetch },
            apikey: env.CUSTOMS_WINDOW_APIKEY,
        }));
        let idmap = env['CUSTOMS_WINDOW_TEST_BULK_UPLOAD_ENTID'];
    ...
    const client = new __1.CustomsWindowSDK({
        base: 'http://localhost:8080',
...
//# sourceMappingURL=BulkUploadDirect.test.js.map