...
(0, node_test_1.describe)('SubmissionDirect', async () => {
    ...
    (0, node_test_1.test)('direct-load-submission', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup({ id: 'direct01' });
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-load-submission', setup.live))
            return;
...
function directSetup(mockres) {
    ...
    if (live) {
        ...
        let idmap = env['CUSTOMS_WINDOW_TEST_SUBMISSION_ENTID'];
