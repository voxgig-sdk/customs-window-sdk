...
class BulkUploadDirectTest < Minitest::Test
  def test_direct_list_bulk_upload
    setup = bulk_upload_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-bulk_upload", setup[:live] ? "live" : "unit")
    if _should_skip
  ...
  def test_direct_load_bulk_upload
    setup = bulk_upload_direct_setup({ "id" => "direct01" })
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-load-bulk_upload", setup[:live] ? "live" : "unit")
    if _should_skip
      skip(_reason || "skipped via sdk-test-control.json")
      return
    end
...
def bulk_upload_direct_setup(mockres)
