...
class PartyEntityTest < Minitest::Test
  ...
  def test_stream
    ...
    seen = base.Party(nil).stream("list", nil, nil).to_a
    ...
    if cfg["feature"].is_a?(Hash) && cfg["feature"].key?("streaming")
      ...
      got = []
      sdk.Party(nil).stream("list", nil, nil).each do |item|
        if item.is_a?(Array)
          got.concat(item)
        else
          got << item
  ...
  def test_basic_flow
    setup = party_basic_setup(nil)
    # Per-op sdk-test-control.json skip.
    _live = setup[:live] || false
    ["create", "list", "update", "load", "remove"].each do |_op|
      _should_skip, _reason = Runner.is_control_skipped("entityOp", "party." + _op, _live ? "live" : "unit")
      if _should_skip
    ...
    # CREATE
    party_ref01_ent = client.Party(nil)
    ...
    party_ref01_list_result = party_ref01_ent.list(party_ref01_match, nil)
    assert party_ref01_list_result.is_a?(Array)
    ...
    party_ref01_ent.remove(party_ref01_match_rm0, nil)

    # LIST
    party_ref01_match_rt0 = {}

    party_ref01_list_rt0_result = party_ref01_ent.list(party_ref01_match_rt0, nil)
    assert party_ref01_list_rt0_result.is_a?(Array)
...
def party_basic_setup(extra)
  Runner.load_env_local

  entity_data_file = File.join(__dir__, "..", "..", ".sdk", "test", "entity", "party", "PartyTestData.json")
