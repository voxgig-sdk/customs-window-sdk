...
module CustomsWindowUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = CustomsWindowResponse.new({})
    ...
    if fetch_err
      ...
      response = CustomsWindowResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = CustomsWindowResponse.new(fetched)
    else
