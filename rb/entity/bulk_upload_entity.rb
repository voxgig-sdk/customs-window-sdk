...
class BulkUploadEntity
  ...
  def make
    opts = @_entopts.dup
    BulkUploadEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = CustomsWindowHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [BulkUpload, Hash] the current BulkUpload data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = CustomsWindowHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of BulkUpload fields)
  def match_get
  ...
  #
  # @param reqmatch [BulkUploadLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.BulkUpload.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [BulkUpload, Hash] the loaded BulkUpload; raises CustomsWindowError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = CustomsWindowHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
  ...
  #
  # @param reqmatch [BulkUploadListMatch, Hash, nil] match filter (any subset of
  #   BulkUpload fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<BulkUpload>, Array] the matching BulkUpload items; raises CustomsWindowError on failure
  ...
  #
  # @param reqdata [BulkUploadCreateData, Hash, nil] body data
  # @param ctrl [Object, nil] optional per-call control
  # @return [BulkUpload, Hash] the created BulkUpload; raises CustomsWindowError on failure
  ...
  #
  # @param reqdata [BulkUploadUpdateData, Hash, nil] body data
  # @param ctrl [Object, nil] optional per-call control
  # @return [BulkUpload, Hash] the updated BulkUpload; raises CustomsWindowError on failure
  ...
  #
  # @param reqmatch [BulkUploadRemoveMatch, Hash, nil] match criteria (id/query fields)
  # @param ctrl [Object, nil] optional per-call control
  # @return [BulkUpload, Hash] the removed BulkUpload; raises CustomsWindowError on failure
