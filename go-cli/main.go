// Command CustomsWindow-cli is a CLI + REPL for the CustomsWindow SDK,
...
const prompt = "customs-window"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "bulk_upload file paginated_bulk_upload_list_list paginated_party_list_list paginated_submission_list_list party submission submission_detail"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      CUSTOMS_WINDOW_APIKEY (api key), CUSTOMS_WINDOW_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: CUSTOMS_WINDOW_APIKEY carries the API key and
	// CUSTOMS_WINDOW_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("CUSTOMS_WINDOW_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("CUSTOMS_WINDOW_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewCustomsWindowSDK(opts)
