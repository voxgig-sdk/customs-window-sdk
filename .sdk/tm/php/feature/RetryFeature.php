...
// CustomsWindow SDK retry feature
...
class CustomsWindowRetryFeature extends CustomsWindowBaseFeature
{
    ...
    public function init(CustomsWindowContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (CustomsWindowContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(CustomsWindowContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(CustomsWindowContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
