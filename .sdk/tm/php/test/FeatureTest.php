...
// CustomsWindow SDK feature test
//
...
class FtCtrl extends CustomsWindowControl
{
...
class FtClient
{
    ...
    private CustomsWindowUtility $utility;

    public function __construct(CustomsWindowUtility $utility, array $options)
    {
    ...
    public function get_utility(): CustomsWindowUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (CustomsWindowContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public CustomsWindowContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (CustomsWindowContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = CustomsWindowConfig::make_feature($name);
    ...
    private static function buildUrl(CustomsWindowSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof CustomsWindowError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new CustomsWindowResponse($fetched);
        }
    ...
    private function fail(CustomsWindowContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (CustomsWindowContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
